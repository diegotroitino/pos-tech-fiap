...
export function MainLayout({ children }: MainLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 transform bg-white shadow-md transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${
          ...
          <ul className="space-y-2">
            {navItems.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={`flex items-center rounded-xl px-4 py-2 text-sm font-medium transition-colors hover:bg-gray-100 ${
                    pathname === item.href ? "bg-gray-100 text-primary" : "text-gray-600"
                  }`}
                >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center space-x-2 rounded-xl">
                  ...
                  <span className="hidden text-sm font-medium md:inline-block">Rafael Almeida</span>
